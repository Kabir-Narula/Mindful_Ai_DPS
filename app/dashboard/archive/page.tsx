...
import { formatInToronto, formatUTCDate } from '@/lib/timezone'
...
export default async function ArchivePage({ searchParams }: { searchParams: { q?: string } }) {
    ...
    // OPTIMIZED: Fetch all data in parallel with single queries instead of N+1
    // This reduces database round trips from 2N+1 to just 3 queries
    const [dayLogs, allJournals, allMoods] = await Promise.all([
    ...
    // Create efficient lookup maps - group entries by date string (Toronto timezone)
    const journalCountByDate = new Map<string, number>()
    const moodCountByDate = new Map<string, number>()

    for (const journal of allJournals) {
        const dateKey = formatInToronto(journal.createdAt, 'yyyy-MM-dd')
        journalCountByDate.set(dateKey, (journalCountByDate.get(dateKey) || 0) + 1)
    }

    for (const mood of allMoods) {
        const dateKey = formatInToronto(mood.createdAt, 'yyyy-MM-dd')
        moodCountByDate.set(dateKey, (moodCountByDate.get(dateKey) || 0) + 1)
    }

    // Build final data with O(1) lookups instead of O(N) queries
    // IMPORTANT: Use formatUTCDate for dayLog.date (stored as UTC midnight)
    // to match the Toronto-formatted entry dates
    const dayLogsWithCounts = dayLogs.map(log => {
        // dayLog.date is stored as UTC midnight (e.g., 2025-12-02T00:00:00.000Z)
        // We need to extract the UTC date string to match Toronto-formatted entries
        const dateKey = formatUTCDate(log.date, 'yyyy-MM-dd')
        return {
            ...log,
            _count: {
                journalEntries: journalCountByDate.get(dateKey) || 0,
                moodEntries: moodCountByDate.get(dateKey) || 0
            },
            ...
            {dayLogsWithCounts.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {dayLogsWithCounts.map((log) => {
                        // Format dates using UTC to match the stored date (which represents the calendar date)
                        const dateForLink = formatUTCDate(log.date, 'yyyy-MM-dd')
                        const dayOfWeek = formatUTCDate(log.displayDate, 'EEEE')
                        const displayDateStr = formatUTCDate(log.displayDate, 'MMM d')

                        return (
                            <Link key={log.id} href={`/dashboard/archive/${dateForLink}`} className="group">
